...
app.get('/', (req, res) => {
    res.json({ success: true, message: "BlackPay Master Server is Live & Running!" });
});

// ============================================================================
// 0. API: SEND SMS VIA FAST2SMS (route=q EXACT MATCH)
// ============================================================================
...
// ============================================================================
// 2. API: VERIFY OTP & EXTRACT OG UPI (STRICT NETWORK SNIFFER)
// ============================================================================
app.post('/api/wallet/verify-otp', async (req, res) => {
    ...
    try {
        ...
        if (currentUrl.includes('freecharge') || currentWallet.includes('freecharge')) { 
            ...
            console.log("[+] Searching OG Paytm UPI...");
            ...
            // 🔴 DUMMY UPI CODE HATA DIYA HAI. AGAR OG NAHI MILI TOH SAAMNE ERROR AAYEGA
            if (!finalUpi || finalUpi.trim() === "") {
                return res.status(400).json({ success: false, message: "Asli (OG) UPI Extraction Failed. Dashboard load nahi hua, wapas try kar." });
            }
        } 
